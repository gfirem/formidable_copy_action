...
class FormidableCopyAction extends FrmFormAction {
	...
	public function admin_style() {
		...
		if ( $current_screen->id === 'toplevel_page_formidable' ) {
			wp_enqueue_style( 'formidable_copy_action', COPY_ACTION_CSS_PATH . 'formidable_copy_action.css' );
		}
	}
		...
		if ( $persit_destination_data ) {
			...
			if ( $jsonData != null ) {
				...
				foreach ( $jsonData as $val ) {
					...
					if ( is_array( $shortCodes ) && ! empty( $shortCodes[2] ) ) {
						$size = count( $shortCodes[2] );
						if ( ! empty( $shortCodes[2][0] ) && $size == 1 ) {
							$key_to_check = $shortCodes[2][0];
							$exists       = in_array( $key_to_check, $meta_key );
							if ( $exists ) {
								if ( ! is_numeric( $key_to_check ) ) {
									$field_id = FrmField::get_id_by_key( $key_to_check );
								} else {
									$field_id = $key_to_check;
								}
	...
	public function get_target_value_for_field( $entry_id, $meta, $persist_fields ) {
		...
		foreach ( $target_entry->metas as $target_key => $target_source_keys ) {
	...
	public function form( $form_action, $args = array() ) {
		...
		if ( $form->status === 'published' ) {
			require COPY_ACTION_VIEW_PATH . 'form.php';
		} else {
			...
			jQuery(document).ready(function ($) {
				...
				jQuery(".frm_single_formidable_copy_settings").each(function () {
					...
					jQuery(".frm_form_settings").submit(function (e) {
						...
						jQuery("[name='frm_formidable_copy_action[" + actionId + "][post_content][form_destination_data]']").val(json);
						console.log();
					});
